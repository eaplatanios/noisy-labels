...
func makeSimpleGraph(_ firstComponentSize: Int, _ secondComponentSize: Int) -> Graph {
  ...
  let features = Tensor<Float>(
    stacking: (0..<firstComponentSize).map { _ in Tensor<Float>([1, 0]) } + (0..<secondComponentSize).map { _ in Tensor<Float>([0, 1]) },
    alongAxis: 0)
  ...
  let labels = [Int](repeating: 0, count: firstComponentSize) + [Int]((0..<secondComponentSize).map { $0 % 2 })
