        ...
        for (index, neighbor) in graph.neighbors[Int(node)].enumerated() {
          for k in 0..<graph.classCount {
            scores[k] += qualityLogits[Int(node)].qualityLogits(
              node: index,
              l: k,
              k: labels[Int(neighbor)])
          }
        }
        let label = scores.indexOfMax!
        if labels[Int(node)] != label { converged = false }
        labels[Int(node)] = label
      }
    }
    if converged {
      // logger.info("ICM converged in \(step) steps.")
      break
    }
  }
  return labels.map(Int32.init)
}
