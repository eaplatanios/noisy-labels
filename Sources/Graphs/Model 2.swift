...
//     var allUnlabeledData = Dataset(elements: graph.allUnlabeledData)
...
//         allUnlabeledData = Dataset(elements: graph.data(atDepth: emStep + 1))
//       }
...
//     var eStepAccumulator = Tensor<Float>(zeros: [predictor.nodeCount, predictor.classCount])
...
//           let hLog = predictions.labelProbabilities                                                 // [BatchSize, MaxNeighborCount, ClassCount, ClassCount]
...
//           let yHat = currentSample.gathering(atIndices: predictions.neighborIndices)                // [BatchSize, MaxNeighborCount]
//           let qLogYHatAll = qLog.batchGathering(
//             atIndices: yHat
//               .expandingShape(at: 2, 3)
//               .tiled(multiples: Tensor<Int32>([1, 1, Int32(predictor.classCount), 1])),
//             alongAxis: 3,
//             batchDimensionCount: 3
//           ).squeezingShape(at: 3)                                                                   // [BatchSize, MaxNeighborCount, ClassCount]
//           let mask = qLogMask.expandingShape(at: -1)                                                // [BatchSize, MaxNeighborCount, 1]
//           let qLogYHat = (qLogYHatAll * mask).sum(squeezingAxes: 1)                                 // [BatchSize, ClassCount]
//           let sample = Tensor<Int32>(
//             randomCategorialLogits: hLog + qLogYHat,
//             sampleCount: 1
//           ).squeezingShape(at: -1)
...
//       print(stackedSamples[0, 0..., 0])
//       print(stackedSamples[Int(graph.neighbors[node][0]), 0..., 0])
//       print((Tensor<Float>(neighborSamples) * Tensor<Float>(nodeSample))[0])
...
//     }
//     print(expectedNodePairs[0]![0])
//     print(expectedNodePairs[0]![0].sum(squeezingAxes: -2))
//   }
...
//           // print(pairExpectations[0])
//           // print(exp(qLog)[0])
//           let modelLoss = -(yExpected * hLog).sum() - (pairExpectations * qLog).sum()

//           // let qLogMask = predictions.qualitiesMask.expandingShape(at: -1)                           // [BatchSize, MaxNeighborCount, 1]
//           // // let neighborValues = exp(predictions.neighborLabelProbabilities)
//           // let yHat = expectedLabels.gathering(
//           //   atIndices: predictions.neighborIndices
//           // ).expandingShape(at: 2)                                                                   // [BatchSize, MaxNeighborCount, 1, ClassCount]
//           // let qLogYHat = ((qLog * yHat).sum(squeezingAxes: -1) * qLogMask).sum(squeezingAxes: 1)    // [BatchSize, ClassCount]
//           // let yExpected = expectedLabels.gathering(atIndices: batch)
//           // // let hLoss = softmaxCrossEntropy(
//           // //   logits: hLog,
//           // //   probabilities: yExpected,
//           // //   reduction: { $0.sum() })
//           // // let qLoss = softmaxCrossEntropy(
//           // //   logits: qLogYHat,
//           // //   probabilities: yExpected,
//           // //   reduction: { $0.sum() })
//           // // let modelLoss = hLoss + qLoss
//           // let modelLoss = -(yExpected * hLog).sum() - (yExpected * qLogYHat).sum()
//           // let maskedQEntropy = exp(qLog) * qLog * qLogMask.expandingShape(at: -1)
...
//           // let loss = modelLoss + hEntropy + qEntropy + compilerBug
//           let loss = modelLoss + hEntropy + compilerBug
//           return loss / Float(predictions.labelProbabilities.shape[0])
//         }
//         optimizer.update(&predictor, along: gradient)
