...
/// Weather sentiment Amazon Mechanical Turk dataset loader.
///
/// Source: https://eprints.soton.ac.uk/376543/1/WeatherSentiment_amt.csv
public struct WeatherSentimentLoader: DataLoader {
  private let url: URL = URL(
    string: "https://dl.dropboxusercontent.com/s/4zrrl7dgxg0g71h/weather_sentiment.zip")!
  ...
  public func load(withFeatures: Bool = false) throws -> Data<Int, String, Int> {
    precondition(
      withFeatures == false,
      "The weather sentiment dataset does not provide instance features.")

    logger.info("Loading the weather sentiment dataset.")

    let dataDir = self.dataDir.appendingPathComponent("weather-sentiment")
    let compressedFile = dataDir.appendingPathComponent("weather-sentiment.zip")
