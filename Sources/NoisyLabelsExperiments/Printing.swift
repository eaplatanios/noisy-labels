...
public struct ResultsPrinter {
  ...
  public func makeTables() throws {
    ...
    //   .predictorCount: ...
    // ]
    var parsed = [ExperimentResult.ParameterType: [DataPartition: [Metric: [String: [Int: [Float]]]]]]()
    for result in results {
      let parts = result.components(separatedBy: "\t")
      if parts.count < 6 { continue }
      let metricParts = parts[4].components(separatedBy: "-")
      if metricParts.count < 2 { continue }
      let l = parts[1]
      let t = ExperimentResult.ParameterType(rawValue: parts[2])!
      let p = Int(parts[3])!
      let partition = DataPartition(rawValue: metricParts[0])!
      let m = Metric(rawValue: metricParts[1])!
      let v = Float(parts[5])!
      if !parsed.keys.contains(t) { parsed[t] = [DataPartition: [Metric: [String: [Int: [Float]]]]]() }
      if !parsed[t]!.keys.contains(partition) { parsed[t]![partition] = [Metric: [String: [Int: [Float]]]]() }
      if !parsed[t]![partition]!.keys.contains(m) { parsed[t]![partition]![m] = [String: [Int: [Float]]]() }
      if !parsed[t]![partition]![m]!.keys.contains(l) { parsed[t]![partition]![m]![l] = [Int: [Float]]() }
      if !parsed[t]![partition]![m]![l]!.keys.contains(p) { parsed[t]![partition]![m]![l]![p] = [Float]() }
      parsed[t]![partition]![m]![l]![p]!.append(v)
    }
    ...
    for (type, partitionResults) in parsed {
      var tables = [Metric: [String]]()
      for (partition, typeResults) in partitionResults {
