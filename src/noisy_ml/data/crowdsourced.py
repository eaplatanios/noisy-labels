...
__all__ = ["BlueBirdsLoader", "SentimentPopularityLoader", "WeatherSentimentLoader"]
  ...
  """
    ...
    def load(data_dir, load_features=True):
        ...
        def convert_labels_to_ints(dictionary):
            return dict(map(lambda kv: (kv[0], int(kv[1])), six.iteritems(dictionary)))
        ...
        def values_to_tuple(values):
            i_ids, values = map(
                list, zip(*[(instance_ids[i], v) for i, v in six.iteritems(values)])
            )
            return i_ids, values

        true_labels = {0: {instance_ids[i]: l for i, l in six.iteritems(ground_truth)}}
        predicted_labels = {
            0: {
...
class SentimentPopularityLoader(object):
  ...
  """
    ...
    def load(data_dir):
        ...
        column_names = ["WorkerID", "TaskID", "Label", "True label", "Judgement time"]
...
class WeatherSentimentLoader(object):
  ...
  """
    ...
    def load(data_dir):
        # Load data.
        datapath = os.path.join(
            data_dir, "crowdsourced", "weather_sentiment", "WeatherSentiment_amt.csv"
        )
        column_names = ["WorkerID", "TaskID", "Label", "True label", "Judgement time"]
        df = pd.read_csv(datapath, names=column_names)
