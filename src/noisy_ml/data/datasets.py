...
TrainData = namedtuple(
    "TrainData", ["instances", "predictors", "labels", "values"]
)
...
class Dataset(object):
        ...
        for dataset in datasets:
            for l_old, label in enumerate(dataset.labels):
                ...
                if l not in true_labels:
                    true_labels[l] = dict()
                for i_old, true_label in six.iteritems(
                    dataset.true_labels[l_old]
                ):
                    ...
                    if p is None:
                        ...
                        if has_pf:
                            predictor_features.append(
                                dataset.predictor_features[p_old]
                            )
    ...
    def compute_confusions(self, eps=1e-10):
        ...
        for l_id, nc in enumerate(self.num_classes):
            ...
            for p_id, indices_values in six.iteritems(
                self.predicted_labels[l_id]
            ):
                ...
                confusions[l_id][p_id] /= (
                    confusions[l_id][p_id].sum(-1, keepdims=True) + eps
                )
