...
class Result(object):
    def __init__(self, mad_error_rank, mad_error, accuracy, auc):
...
class Evaluator(object):
    ...
    def evaluate_per_label(self, learner, batch_size=128):
        ...
        predicted_qualities = np.mean(
            learner.qualities(instances, predictors, labels, batch_size=batch_size),
            axis=0,
        )
            ...
            pq = np.array(
                list(
                    map(
                        lambda kv: kv[1],
                        sorted(
                            six.iteritems(predicted_qualities), key=lambda kv: kv[0]
                        ),
                    )
                )
            )
