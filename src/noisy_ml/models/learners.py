...
class EMLearner(Learner):
    ...
    def _m_step(
        self, iterator_init_op, warm_start, max_m_steps, log_m_steps=None
    ):
        self._session.run(iterator_init_op)
        if not warm_start:
        ...
        for m_step in range(max_m_steps):
            ...
            if log_m_steps is not None and (
                ...
                logger.info(
                    "Step: %5d | Negative Log-Likelihood: %.8f" % (m_step, nll)
                )
                accumulator_nll = 0.0
                accumulator_steps = 0
    ...
    def train(
        ...
        for em_step in em_steps_range:
            ...
            self._m_step(
                m_step_iterator_init_op, warm_start, max_m_steps, log_m_steps
            )
    ...
    def qualities(self, instances, predictors, labels, batch_size=128):
        ...
        if isinstance(qualities_mean_log[0], list):
            ...
            for l, q in zip(
                range(self.config.num_labels), zip(*qualities_mean_log)
            ):
...
class MultiLabelEMConfig(EMConfig):
    ...
    def build_ops(self):
        ...
        h_log = tf.squeeze(
            tf.batch_gather(
                params=h_log, indices=tf.expand_dims(l_indices, axis=-1)
            ),
            axis=-1,
        )
        ...
        with tf.name_scope("e_step"):
            ...
            if isinstance(self.model, MMCE_M):
                ...
                e_y = tf.cast(
                    tf.greater_equal(tf.exp(e_y_log), 0.5), e_y_log.dtype
                )
...
class MultiLabelMultiClassEMConfig(EMConfig):
    ...
    def build_ops(self):
        ...
        l_indices_onehot = tf.unstack(
            tf.one_hot(l_indices, self.num_labels), axis=-1
        )
        ...
        for y_hat, q_log in zip(y_hats, q_params):
            q_log_y_hats.append(
                tf.einsum("ijk,ik->ij", q_log, tf.to_float(y_hat))
            )
        ...
        with tf.name_scope("e_step"):
            ...
            e_y_acc_updates = [
                tf.cond(
                    use_maj, lambda: tf.to_float(y_hat), lambda: q_log_y_hat
                )
            ...
            # list of <float32> [batch_size_l, num_classes_l] for each label l.
            e_y_as = [
                tf.gather(e_y_acc, xli)
                for xli, e_y_acc in zip(xl_indices, e_y_accs)
            ]
            # list of <float32> [batch_size_l, num_classes_l] for each label l.
            e_y_a_h_logs = e_y_as
            # if include_y_prior:
            ...
            e_y_logs = [
                tf.stop_gradient(
                    tf.cond(
                        ...
                        lambda: e_y_a_h_log
                        - tf.reduce_logsumexp(
                            e_y_a_h_log, axis=-1, keepdims=True
                        ),
                    )
                )
