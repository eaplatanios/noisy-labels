"""A collection of custom TF layers."""
...
class FeatureMap(Layer):
    def __init__(self, features, adjust_magnitude=False, name="FeatureMap"):
...
class OneHotEncoding(Layer):
    def __init__(self, num_inputs, dtype=tf.float32, name="OneHotEncoding"):
...
class Embedding(Layer):
    def __init__(self, num_inputs, emb_size, dtype=tf.float32, name="Embedding"):
        self.num_inputs = num_inputs
...
class Linear(Layer):
    ...
    def apply(self, *args, **kwargs):
        inputs = args[0]
        w_initializer = tf.glorot_uniform_initializer(dtype=inputs.dtype)
        b_initializer = tf.zeros_initializer(dtype=inputs.dtype)
        with tf.variable_scope(self.name):
            w = tf.get_variable(
                name="weights",
            ...
            b = tf.get_variable(
                name="bias", shape=[self.num_outputs], initializer=b_initializer
            )
            outputs = tf.nn.xw_plus_b(x=inputs, weights=w, biases=b, name="linear")
        return outputs
...
class MLP(Layer):
  ...
  """
    ...
    def apply(self, *args, **kwargs):
        with tf.variable_scope(self.name):
            hidden = args[0]
            w_initializer = tf.glorot_uniform_initializer(dtype=hidden.dtype)
            b_initializer = tf.zeros_initializer(dtype=hidden.dtype)
            ...
            for i, num_units in layers:
                with tf.variable_scope("layer_{}".format(i)):
                    ...
                    b = tf.get_variable(
                        name="bias", shape=[num_units], initializer=b_initializer
                    )
                    if hidden.shape.ndims == 3:
                        hidden = (
                            tf.tensordot(hidden, w, axes=[[2], [0]], name="linear") + b
                        )
                    else:
    ...
    def apply(self, *args, **kwargs):
        with tf.variable_scope(self.name):
            ...
            for c_f, c_k, p_f, p_k in layers:
                hidden = tf.keras.layers.Conv2D(c_f, c_k, activation=self.activation)(
                    hidden
                )
...
class LogSigmoid(Layer):
    ...
    def apply(self, *args, **kwargs):
        ...
        with tf.variable_scope(self.name):
            ...
            b = tf.get_variable(
                name="bias", shape=[self.num_labels], initializer=b_initializer
            )
            outputs = tf.nn.xw_plus_b(x=inputs, weights=w, biases=b, name="linear")
...
class HierarchicalLogSoftmax(Layer):
    def __init__(self, num_labels, hierarchy, name="HierarchicalLogSoftmax"):
        self.num_labels = num_labels
        self.hierarchy = hierarchy
        self.name = name

    def apply(self, *args, **kwargs):
        return self._compute(args[0], self.hierarchy, parent=0, level=0)
    ...
    def _compute(self, inputs, hierarchy, level, parent):
        ...
        with tf.variable_scope("%s_%d_%d" % (self.name, level, parent)):
            ...
            b = tf.get_variable(
                name="bias", shape=[num_parents], initializer=b_initializer
            )
            outputs = tf.nn.xw_plus_b(x=inputs, weights=w, biases=b, name="linear")
            ...
            for i, p in enumerate(parents):
                ...
                if len(p_hierarchy) > 0:
                    p_output = self._compute(inputs, p_hierarchy, level + 1, p)
...
class DecodeJpeg(Layer):
    ...
    def apply(self, *args, **kwargs):
        ...
        return tf.map_fn(
            lambda i: tf.cast(
                tf.reshape(
                    tf.image.decode_jpeg(contents=i, channels=3),
